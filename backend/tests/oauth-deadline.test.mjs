...
test('slow profile body is bounded and cleanup receives a fresh unexpired signal', async () => {
  ...
  }, { workMs: 30, cleanupMs: 50 });
  await assert.rejects(api.exchangeCodeForProfile(input), { code: 'deadline_exceeded' });
...
test('insufficient Lambda time rejects before requesting upstream tokens', async () => {
  ...
  await assert.rejects(api.exchangeCodeForProfile({ ...input, remainingTimeMs: () => 2_500 }),
    { code: 'deadline_exceeded' });
});

test('valid bounded flow returns identity only after successful cleanup', async () => {
  let revoked = false;
  const api = new RobloxOAuthApi(async (url) => {
    if (url.endsWith('/token')) return tokens();
    if (url.endsWith('/userinfo')) return Response.json({ sub: '123' });
  ...
  assert.equal((await api.exchangeCodeForProfile(input)).sub, '123');
