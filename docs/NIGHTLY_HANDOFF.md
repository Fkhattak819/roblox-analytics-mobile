...
- Replaced operator-created `ACCESS#` grants and the runtime Open Cloud API key with Roblox OAuth delegation. The app now requests `openid profile universe.analytics:read`, calls Roblox's token-resources endpoint, intersects concrete authorized universe IDs with the deployment allowlist, and returns those IDs in the app session. The fixed development universe remains `10009166512`.
...
- Official Roblox docs rechecked: OAuth access tokens are 15 minutes, refresh tokens are 90 days and single-use, token-resources supplies permitted resources, and `universe.analytics:read` covers Analytics Query endpoints. The implementation rejects wildcard-only/unknown resources and fails closed when no concrete allowlisted universe is present.
...
- Exact next action: after the user confirms the pending Roblox permission edit, add and save only `universe.analytics:read`; then commit/push, deploy `roblox-analytics-mobile-dev`, verify CloudFormation/KMS/IAM/Lambda/alarms and live HTTP boundaries, and complete one browser OAuth consent for universe `10009166512`. The SNS email subscription at `fkhattak819@gmail.com` still needs the recipient's confirmation link.
