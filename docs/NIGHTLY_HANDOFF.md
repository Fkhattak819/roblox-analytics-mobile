...
- Preserved compatibility-sensitive `roblox-analytics-mobile` identifiers, including package names, SecureStore and preference keys, OAuth scheme/service constants, AWS resource names, metric namespaces, GitHub paths, and historical records. Renaming those identifiers requires a separately planned migration rather than a display-name change.
- The connected Figma file and historical records retain their original StudioPulse names; no Figma or cloud mutation was performed.
...
- Connected onboarding, Home, Analytics, analytics details, Experiences, Sales, Profile, Connections, Data Coverage, More, and the experience picker now use the current session's `authorizedUniverseIds`. The first authorized universe is selected automatically and testers can switch only among IDs present in their own session. Unknown experiences use a truthful `Universe <id>` label and neutral Roblox Analytics Studio artwork until a reviewed metadata inventory route exists.
...
- Added one TLS-only SNS topic as the action for both application alarms. Both alarms report `OK` and each has one action. The approved email subscription is `PendingConfirmation`; the recipient must click the AWS confirmation email before alarm mail can be delivered. These are Roblox Analytics Studio authentication/runtime alarms, not blanket monitoring for every AWS service or application in the account.
