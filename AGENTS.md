...
Read `docs/EXPO_DEVELOPMENT_PLAYBOOK.md`, `docs/API_CONTRACT.md`, `docs/FIGMA_IMPLEMENTATION_MANIFEST.md`, and `design-system-state-studiopulse-onboarding.json` before making product or architecture changes. `docs/MAC_IOS_DEVELOPMENT_PLAYBOOK.md` is historical SwiftUI/Tuist planning context; when it conflicts with the active Expo implementation, the Expo repository, Expo playbook, and current API contract win. Before implementing or visually approving a screen, also read its current live Figma node through the connected Figma tools.

## Product boundaries

- Build an Expo-managed React Native app with TypeScript. Use Expo Go for fast local iteration, then an Expo development build or EAS Build when native modules, custom app configuration, remote push notifications, or production packaging require it. iOS is the primary target; Android and web are out of scope unless the user explicitly reopens them.
...
- Keep the build loop CLI-first with `npm`, `npx expo`, and EAS CLI, plus TypeScript checks, lint, unit tests, an iOS export, and the iPhone simulator. Do not hand-edit or generate native projects when app config or a reviewed config plugin can express the change.
- Prefer React Native and Expo primitives, Expo Router, Expo SecureStore, Expo AuthSession, Expo Linking, `fetch`, TypeScript, and small testable service/repository modules.
...
- Sample mode and deterministic fixtures must remain usable without network access, AWS, Roblox credentials, paid Expo/EAS services, or an Apple Developer Program membership.
...
- After each implementation checkpoint, run the smallest relevant Expo build/test. Before claiming a UI slice complete, launch it in Expo Go or an Expo development build on an available device or iPhone simulator, capture visual evidence at matching geometry, and compare it with the exact live Figma frame using an overlay or image diff. Record any intentional platform-rendering tolerance.
