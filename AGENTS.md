...
Read `docs/MAC_IOS_DEVELOPMENT_PLAYBOOK.md`, `docs/FIGMA_IMPLEMENTATION_MANIFEST.md`, and `design-system-state-studiopulse-onboarding.json` before making product or architecture changes. Before implementing or visually approving a screen, also read its current live Figma node through the connected Figma tools.

## Product boundaries

- Build a native iOS app with SwiftUI. Android and web are out of scope unless the user explicitly reopens them.
...
- Keep the iOS build loop CLI-first with Tuist and `xcodebuild`. Do not hand-edit a generated `.xcodeproj`.
- Prefer Apple frameworks: SwiftUI, Observation, Swift Concurrency, Swift Charts, URLSession, Codable, OSLog, Swift Testing, and XCTest/XCUITest.
- Follow the installed `figma-design-to-code` and `figma-swiftui` workflows for every Figma-derived screen. Call live design context on the exact frame before writing its view, using Swift and SwiftUI as the client language/framework, then retrieve an exact-node Figma screenshot for that variant. Page metadata may be used to locate frames, but metadata, screenshots, and the local JSON must never replace design context.
- When live design context reports motion, retrieve the recursive motion context before implementing it. Translate the returned timing, easing, transforms, and animated node IDs into native SwiftUI while honoring Reduce Motion.
- Download and preserve the exact assets returned by Figma before their temporary URLs expire. Reuse Figma components and semantic tokens where possible; treat generated web markup only as structural reference and write idiomatic SwiftUI.
...
- The iOS app may store StudioPulse session tokens in Keychain. It must never store a Roblox Open Cloud API key.
